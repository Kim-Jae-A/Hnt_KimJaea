...
namespace MODBUS_TEST_2
{
    public partial class Sensor2 : Form
    {
        ...
        {
            Sensor2_Sock = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        }

        private void ConnServer()
        {       
            // 서버 아이피
            //string serverip = "10.10.24.252";
            ...
            try
            {
                ...
                Sensor2_Sock.BeginConnect(clientEP, new AsyncCallback(ConnectCallback), Sensor2_Sock);      // 접속한 서버 IP 에 맞게 소켓 연결
            }
            catch (Exception ex)
            {
        ...
        void ConnectCallback(IAsyncResult ar)   //커넥트 콜백 메소드
        {
            try
            {
                ...
                obj.WorkingSocket = Sensor2_Sock;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{DateTime.Now}" + ex);
        ...
        public void Send(string msg)   // 서버로 데이터 보내는 메소드
        {
            try
            {
                ...
                Sensor2_Sock.Send(buff, buff.Length, 0);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{DateTime.Now}" + ex);
