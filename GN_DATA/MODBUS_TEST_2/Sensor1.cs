    ...
    public partial class Sensor1 : Form
    {
        ...
        private void ConnServer()
        {
            ...
            try
            {
                ...
                Sensor1_Sock.BeginConnect(clientEP, new AsyncCallback(ConnectCallback), Sensor1_Sock);      // 접속한 서버 IP 에 맞게 소켓 연결
            }
            catch (Exception ex)
            {
...
/*                thread_Tem = new Thread(Workthread_RESIVE);    // 쓰레드 설정                                           
        ...
        void ConnectCallback(IAsyncResult ar)   // 콜백 메소드
        {
            try
            {
                ...
                obj.WorkingSocket = Sensor1_Sock;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{DateTime.Now}" + ex);
        ...
        public void Send(string msg)   // 서버로 데이터 보내는 메소드
        {
            try
            {
                ...
                Sensor1_Sock.Send(buff, buff.Length, 0);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"{DateTime.Now}" + ex);
        ...
        /*private void Workthread_RESIVE()   // 셋팅한 쓰레드가 시작되면 실행되는 함수
        {
            while (true)
            {
                try
                {
                    Sensor1_Sock.BeginReceive(obj.Buffer, 0, obj.BufferSize, 0, DataReceived, obj);  // 실시간으로 서버에서 보낸 데이터를 받는다.
                }
                catch (Exception ex)
                {
                    obj.ClearBuffer();
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                Sensor1_Sock.BeginReceive(obj.Buffer, 0, obj.BufferSize, 0, DataReceived_per, obj);  // 실시간으로 서버에서 보낸 데이터를 받는다.
            }
            catch (Exception ex)
            {
                obj.ClearBuffer();
